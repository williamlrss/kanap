...
export default class Cart {
	...
	classCartAdd(productToAdd) {
		// Validate that the product being added is a valid object with required properties
		if (!productToAdd || typeof productToAdd !== 'object' || productToAdd instanceof String || productToAdd instanceof Number) {
			const errorMessage = `Invalid productToAdd: must be an object with properties _id (string), color (string), name (string) and quantity (number). Received: ${productToAdd}`;
			console.error(errorMessage);
			return false;
		}

		// Validate the required properties of the product being added
		const { _id, color, quantity, name } = productToAdd;
		if (!_id || typeof _id !== 'string' || _id.trim().length === 0) {
			console.error('Invalid productToAdd: _id must be a non-empty string');
			return false;
		}
		if (!color || typeof color !== 'string' || color.trim().length === 0) {
			console.error('Invalid productToAdd: color must be a non-empty string');
			return false;
		}
		if (!quantity || typeof quantity !== 'number' || isNaN(quantity)) {
			console.error('Invalid productToAdd: quantity must be a number');
			return false;
		}
		if (!name || typeof name !== 'string' || name.trim().length === 0) {
			console.error('Invalid productToAdd: name must be a non-empty string');
			return false;
		}
		...
		if (foundProductInCart) {
			foundProductInCart.quantity = parseInt(foundProductInCart.quantity) + parseInt(productToAdd.quantity);
		}
	...
	classCartRemove(productToRemove) {
		// Validate that the product being removed is a valid object with required properties
		if (!productToRemove || typeof productToRemove !== 'object' || productToRemove instanceof String || productToRemove instanceof Number) {
			console.error(
				`Invalid productToRemove: must be an object with properties _id (string), color (string), and quantity (number). Received: ${productToRemove}`
			);
			return false;
		}

		// The deletion is done by creating an identical cart, filtered by omitting this product.
		this.cart = this.cart.filter((productsInCart) => productsInCart._id !== productToRemove._id || productsInCart.color !== productToRemove.color);

		/* if ((p) => p._id !== productToRemove._id || p.color !== productToRemove.color) returns TRUE --> productsInCart are saved in new cart
		if ((p) => p._id !== productToRemove._id || p.color !== productToRemove.color) returns FALSE --> productToRemove is omitted
		In our case, only one product will return false for both '||' statement --> the one to delete */

		this.save();
		showAlert(`Produit supprimé du panier : ${productToRemove.name} | ${productToRemove.color}`);
	}
	...
	classCartNewQuantity(productInCart) {
		...
		if (!productInCart || typeof productInCart !== 'object' || productInCart instanceof String || productInCart instanceof Number) {
			console.error(
				`Invalid productInCart: must be an object with properties _id (string), color (string), and quantity (number). Received: ${productInCart}`
			);
			return false;
		}
